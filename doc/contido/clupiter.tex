...
En cuanto a la marca del cluster, éste no puede quedar sin un logo, que se muestra en la Figura \ref{fig:clupiter_logo}.
...
\begin{itemize}
  \item\textbf{OpenMPI}\footnote{\url{https://www.open-mpi.org}}: Implementación libre mantenida por un consorcio de académicos, investigadores y socios que ofrece elevada eficiencia y flexibilidad a quien la implementa. Además es la librería \acrshort{mpi} que ofrece Arch Linux por defecto en sus repositorios, y contra la que se compilan por defecto sus ejecutables.
...
\subsubsection{Ejecución de programas MPI}
